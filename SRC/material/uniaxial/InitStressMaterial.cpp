...
{
  InitStressMaterial *theCopy = 0;
  if (theMaterial)
    theCopy = new InitStressMaterial(this->getTag(), *theMaterial, sigInit);
...
{
  if (theMaterial == 0) {
    opserr << "InitStressMaterial::sendSelf() - theMaterial is null, nothing to send\n";
    return -1;
  }
